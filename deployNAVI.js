...
//let code = '0x' + NaviToken.bytecode; 
//let code = NaviToken.bytecode;
let code = NaviToken.unlinked_binary;
console.log("code = " + code)

// Create Contract proxy class
let NaviContract = web3.eth.contract(NaviToken.abi);
...
console.log("ethBal = " + ethBal + ' ethers');

let estimatedGas = web3.eth.estimateGas({data: code});
console.log("estimatedGas = " + estimatedGas);
estimatedGas = estimatedGas + parseInt(estimatedGas/4);
console.log("estimatedGas augmented= " + estimatedGas);

let gasLimit = web3.eth.getBlock("pending").gasLimit;
console.log("gasLimit = " + gasLimit);

console.log("Deploying the contract from :" + web3.eth.coinbase);
let contract = NaviContract.new({from: web3.eth.coinbase, gas: estimatedGas, gasLimit: gasLimit, data: code});
...
async function waitBlock() {
  while (true) {
    console.log("calling getTransactionReceipt ...");
    let receipt = web3.eth.getTransactionReceipt(contract.transactionHash);
    console.log("... getTransactionReceipt done.");
